...
def label_refine(label, keeponlydim = False):
    # if 'ConfidenceBall1' in label:
    #     label = label.replace('ConfidenceBall1', 'CBall1')
    if '_2m' in label: 
        label = label.replace('_2m', '')
    label_splitted = label.split('_')
    if keeponlydim:
        if len(label_splitted) >= 2:
            label = label_splitted[-1]
    else:
        if len(label_splitted) > 2:
...
def plot_best_results(PATH, dataset, k=projected_dim, m=sketch_dim, box_plot=False):
    ...
    scatter_dims = {'CBRAP':[k, ], 'ConfidenceBall1_FJLT':[k, ],
                    'CBSCFD':[m, ], 'SOFUL_2m':[m, ] }
    scatter_T_interval = np.array([ 0, ])
    ...
    for model_name in models_names:
        ...
        if model_name in ['Random', 'LinUCB', 'ConfidenceBall1']:
            try: 
                ...
                for t in T_s:
                    x = time_cpu_over_t[model_name] 
                    scatter_time[model_name]['x'].append(x[t-1])
                    scatter_time[model_name]['y'].append(regret_mean_over_t[model_name][t-1])
                    scatter_time[model_name]['yerr'].append(regret_std_over_t[model_name][t-1])

                    x =  memory_over_t[model_name] 
                    scatter_mem[model_name]['x'].append(np.max(x[1:]))
                    scatter_mem[model_name]['y'].append(regret_mean_over_t[model_name][t-1])
                    scatter_mem[model_name]['yerr'].append(regret_std_over_t[model_name][t-1])
        ...
        ###### Other models
                ...
                try:
                    ...
                    if new_dim in scatter_dims[model_name]:
                        ...
                        for t in T_s:
                            x = time_cpu_model 
                            scatter_time[model_name]['x'].append(x[t-1])
                            scatter_time[model_name]['y'].append(regret_mean_model[t-1])
                            scatter_time[model_name]['yerr'].append(regret_std_model[t-1])


                            x = memory_model 
                            scatter_mem[model_name]['x'].append(np.max(x[1:]))
                            scatter_mem[model_name]['y'].append(regret_mean_model[t-1])
                            scatter_mem[model_name]['yerr'].append(regret_std_model[t-1])
            ...
            if box_plot_dim != [] and len(box_plot_dim) == len(models_box_plot_dim):
                ...
                ax1_dim.tick_params(axis='both', which='major', labelsize=size_ticks)
                ax1_dim.set_ylabel('Cumulative Regret', fontsize=size_labels_axis)
                ...
                ax3_dim.tick_params(axis='both', which='major', labelsize=size_ticks)
                ax3_dim.set_xlabel('T', fontsize=size_labels_axis)
                ax3_dim.set_ylabel('Time (sec)', fontsize=size_labels_axis)
                # ax3_dim.set_yscale('log')
                ax3_dim.grid(linestyle = '--', linewidth = 0.5)
                # ax3_dim.legend( fontsize=15)
                ...
                fig_dim.legend(lines, labels, ncol=ncol, fontsize=size_legend, loc='upper center')
    ...
    for _, model_name in enumerate(models_names):
        try:
            ...
            if model_name not in list_models_to_not_plot:
                ax1_all.plot(time_step, regret_mean_over_t[model_name], label=label_refine(label_over_t[model_name]), color=colors[model_name], marker=markers[model_name], markersize=8, markevery=int(spacing[model_name]*T/10), alpha=1, )
                ...
                lines += ax2_all.plot(time_step, time_cpu_over_t[model_name], label=label_refine(label_over_t[model_name]), color=colors[model_name], marker=markers[model_name], markersize=8, markevery=int(spacing[model_name]*T/10), alpha=1,)
    ...
    #### Figure 2: Box plot
    fig_box_plot, ax= plt.subplots(1, 1, figsize=(6, 4))
    ...
    size_ticks = 23
    size_legend = 22
    ax1_all.tick_params(axis='both', which='major', labelsize=size_ticks)
    ax1_all.set_xlabel('T',fontsize=size_labels_axis)
    ax1_all.set_ylabel('Cumulative Regret',fontsize=size_labels_axis)
    ...
    # ax2_all.set_title(f'')
    ax2_all.tick_params(axis='both', which='major', labelsize=size_ticks)
    ax2_all.set_xlabel('T', fontsize=size_labels_axis)
    ax2_all.set_ylabel('CPU Time (sec)',fontsize=size_labels_axis)
    ax2_all.set_yscale('log')
    ax2_all.grid(linestyle = '--', linewidth = 0.5)
    # ax2_all.legend(loc='upper center', fontsize=12)  
    ...
    ax1.set_xlabel('CPU Time (sec)', fontsize=23)
    ax1.set_ylabel('Cumulative regret', fontsize=23,)
    ax1.set_xscale('log')
    ...
    # ax1.legend(loc='upper left',bbox_to_anchor=(1, 1.8),   fancybox=True, shadow=True, fontsize=17, ncol=2)


    fig_scatter.tight_layout(pad=1)
    # labels = [l.get_label() for l in lines]
    # for l in lines:
    #     print(type(l), l.get_label() )
    print(labels)
    fig_scatter.legend(handles, labels , ncol=3, fontsize=19, loc='upper center',)
    # fig_all.suptitle(title, fontsize=17)
...
def plot_over_params(PATH, dataset, params=['scale', 'lam'], k=projected_dim, m=sketch_dim,):
    """
        Plot cumulative regret variations for each model over a certain parameter
    """
    ### Get list of models in specified path
    ...
    for i, param in enumerate(params):
        ...
        for model_name in models_names:
            try:
                #### Best over m
                # Regret
                lines += axs[i].plot(vals_in_common, regret_mean_over_param[model_name], label=label_refine(label_over_param[model_name]), marker=markers[model_name], markersize=8, color=colors[model_name])

                axs[i].fill_between(vals_in_common, np.array(regret_mean_over_param[model_name]) - np.array(regret_std_over_param[model_name]), 
                                    np.array(regret_mean_over_param[model_name]) + np.array(regret_std_over_param[model_name]), alpha=0.2, color=colors[model_name])
        ...
        # axs[i].set_title(f'{dataset} - Cumulative Regret Variations Over Parameter')
        if param == 'lam':
            param = 'λ'
        size_label_axis = 23
        size_ticks = 23

        axs[i].set_xlabel(param, fontsize=23)
        if i == 0 :
            axs[i].set_ylabel('Cumulative Regret', fontsize=size_label_axis)
        axs[i].grid(linestyle = '--', linewidth = 0.5)
        axs[i].set_xscale('log')
        # axs[i].set_yscale('log')

        axs[i].tick_params(axis='both', which='major', labelsize=size_ticks)
        # ax1.set_xlim(left=1)
        # ax1.set_ylim(bottom=1)
    size_legend = 24
    labels = [l.get_label() for l in lines]
...
def plot_memory(results, PATH='./', init=False):
    ...
    res = {}
    sorted_res = {}
    models = ['ConfidenceBall1', 'LinUCB',  'ConfidenceBall1_FJLT', 'CBRAP','CBSCFD', 'SOFUL_2m',  ]
    for i, dataset in enumerate(datasets):
        ...
        sorted_res[dataset] = res[dataset]
    ...
    ax.set_ylabel("Peak memory (MiB)", fontsize=23) 
    ax.set_yscale("log")
    # ax.set_title("Peak of memory allocation " + title) 
    ax.grid(linestyle = '--', linewidth = 0.5)
    # ax.legend() 
    ax.legend(loc='upper center',bbox_to_anchor=(0.5, 1.3),   fancybox=True, shadow=True, fontsize=19, ncol=3)
...
def plot_ctr(PATH, dataset, k=projected_dim, m=sketch_dim, box_plot=False):
    ...
    for _, model_name in enumerate(models_names):
        # try:
            ...
            if model_name not in list_models_to_not_plot:
                ax1_all.plot(time_step, rewards, label=label_refine(label_over_t[model_name]), color=colors[model_name], marker=markers[model_name], markersize=8, markevery=int(spacing[model_name]*T/10), alpha=1)
    ...
    ax1_all.set_xlabel('T', fontsize=size_labels_axis)
    ax1_all.set_ylabel('Click Through Rate', fontsize=size_labels_axis)
    ax1_all.tick_params(axis='both', which='major', labelsize=size_ticks)
    # ax1_all.set_yscale('log')
    # ax1_all.set_xlim(left=1)
    # ax1_all.set_ylim(bottom=1)
